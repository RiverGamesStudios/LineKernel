...
void ide_read_buffer(unsigned char channel, unsigned char reg, unsigned int buffer,
	unsigned int quads)
{
	/* WARNING: This code contains a serious bug. The inline assembly trashes ES and ESP for all of 
...
unsigned char ide_print_error(unsigned int drive, unsigned char err)
{
	...
	kprintf("- [%s %s] %s\n", (const char *[]) { "Primary", "Secondary" }[ide_devices[drive].Channel],	// Use 
																										// the 
																										// channel 
																										// as 
																										// an 
																										...
																										// the 
																										// array
		(const char *[]) { "Master", "Slave" }[ide_devices[drive].Drive],	// Same as above, using 
																			// the drive
...
=======
			kprintf("   Found %s Drive %dGB - %s\n", (const char *[]) { "ATA", "ATAPI" }[ide_devices[i].Type],	/* Type 
																												 */
				ide_devices[i].Size / 1024 / 1024 / 2,	/* Size */
>>>>>>> 5d0bc5d6f55310cfbc0343c2b106b90c454c3b07
				ide_devices[i].Model);
		}
}
...
{
	return ide_devices[device].Reserved == 1;
}

const char* ide_access_device_ata_pi(int device)
{
	return (const char *[]) { "ATA", "ATAPI" }[ide_devices[device].Type];
}
